...
try {
  ...
  const args = windows
    ? ["/d", "/s", "/c", "pnpm exec playwright test --config playwright.fullstack.config.ts e2e/publishing.fullstack.spec.ts e2e/revalidation.fullstack.spec.ts"]
    : ["exec", "playwright", "test", "--config", "playwright.fullstack.config.ts", "e2e/publishing.fullstack.spec.ts", "e2e/revalidation.fullstack.spec.ts"];
